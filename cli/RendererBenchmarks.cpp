...
namespace benchmark {
    ...
    std::pair<GraphBuildFn, GraphRenderFn> makeRenderSineNoKeyBenchmark(
        ...
        return makeNativeGraphFns<FloatType>(runtime, [](const size_t i) {
    ...
    template std::pair<GraphBuildFn, GraphRenderFn> makeRenderSineNoKeyBenchmark<float>(const std::shared_ptr<elem::Runtime<float>>&);
    template std::pair<GraphBuildFn, GraphRenderFn> makeRenderSineNoKeyBenchmark<double>(const std::shared_ptr<elem::Runtime<double>>&);
